...
struct Piece {
  ...
  // attack from specific position by this type of piece
  inline constexpr piece_bitboard_t get_moves(pos_t pos, piece_bitboard_t friends, piece_bitboard_t foes, piece_bitboard_t attack_mask=0x00) const {
    ...
    if(value==KING  &&color==WHITE)return Moves<KING  ,WHITE>::get_moves(pos,friends,foes, attack_mask);
    ...
    if(value==KING  &&color==BLACK)return Moves<KING  ,BLACK>::get_moves(pos,friends,foes, attack_mask);
    return 0x00ULL;
  }
