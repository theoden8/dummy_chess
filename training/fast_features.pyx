...
# Declare the C++ function from standalone header
cdef extern from "halfkp_extract.hpp" namespace "preprocess":
...
def get_halfkp_features_arrow(fen_array, bint flip=False):
    ...
    # Pre-allocate output arrays (max 32 pieces per position)
    cdef numpy.ndarray[int32_t, ndim=1] white_indices = numpy.empty(n_positions * 32, dtype=numpy.int32)
    cdef numpy.ndarray[int32_t, ndim=1] black_indices = numpy.empty(n_positions * 32, dtype=numpy.int32)
    ...
    for i in range(n_positions):
        ...
        # Get bytes for this position (as_py returns Python bytes)
        py_val = fen_array[i].as_py()
        if py_val is None:
            stm_ptr[i] = 0
            continue
        
        val = py_val
        data_ptr = <const uint8_t*>val
        data_len = len(val)
        
        # Call C++ feature extraction (nogil for the inner loop)
        with nogil:
